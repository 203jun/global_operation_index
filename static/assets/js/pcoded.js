...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // search-bar (guard when elements are absent)
    var popSearchEl = document.querySelector(".pop-search");
    if (popSearchEl) {
        popSearchEl.addEventListener('click', function () {
            var searchBar = document.querySelector(".search-bar");
            if (searchBar) {
                slideDown(searchBar, 200);
                var inputEl = searchBar.querySelector("input");
                if (inputEl) inputEl.focus();
            }
        });
    }
    var searchBarCloseEl = document.querySelector(".search-bar .btn-close");
    if (searchBarCloseEl) {
        searchBarCloseEl.addEventListener('click', function () {
            var searchBar = document.querySelector(".search-bar");
            if (searchBar) {
                slideUp(searchBar, 200);
            }
        });
    }
    if (document.querySelector('.pcoded-navbar').classList.contains('theme-horizontal')) {
    ...
    // remove pre-loader start (guard when preloader is removed from DOM)
    setTimeout(function () {
        var loaderBg = document.querySelector('.loader-bg');
        if (loaderBg && typeof loaderBg.remove === 'function') {
            loaderBg.remove();
        }
    }, 400);
